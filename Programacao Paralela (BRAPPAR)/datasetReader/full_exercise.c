...
void updateIngredientCount(IngredientCount *ingredientsCounts, int *size, const char *ingredient, const int count) {
    ...
    if (*size < MAX_INGREDIENTS) {
        ...
        // printf("Erro: o número máximo de ingredientes (%d) foi atingido. O ingrediente '%s' não pôde ser adicionado.\n", MAX_INGREDIENTS, ingredient);
    }
}
...
int main(int argc, char **argv) {
    ...
    IngredientCount typedIngredientCount;
    if (rank == 0) {
        printf("Digite o nome do ingrediente que deseja contar: ");
        fgets(typedIngredientCount.ingredient, INGREDIENT_MAX, stdin);
        typedIngredientCount.ingredient[strcspn(typedIngredientCount.ingredient, "\n")] = 0;
        typedIngredientCount.count = 0;
    }
    MPI_Bcast(&typedIngredientCount, sizeof(IngredientCount), MPI_BYTE, 0, MPI_COMM_WORLD);
        ...
        for (int i = 0; i < size; i++) {
            for (int j = 0; j < MAX_INGREDIENTS; j++) {
                if (globalCounts[i * MAX_INGREDIENTS + j].count > 0) {
                    ...
                    if (strcmp(typedIngredientCount.ingredient, globalCounts[i * MAX_INGREDIENTS + j].ingredient) == 0) {
                        typedIngredientCount.count += globalCounts[i * MAX_INGREDIENTS + j].count;
                        // printf("rank %d, tem %d \n\n", i, globalCounts[i * MAX_INGREDIENTS + j].count);
                    }
                }
            }
        }
